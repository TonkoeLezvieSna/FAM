...
# Функция валидации XML
def validate_xml(file_path: str) -> bool:
    """
    Проверяет, является ли файл корректным XML-документом.
    Возвращает True, если парсинг успешен, иначе False.
    Логирует все ошибки с уровнем WARNING.
    """
    try:
        # Пытаемся открыть и распарсить файл
        with open(file_path, 'rb') as f:
            xml.dom.minidom.parse(f)
        return True
    except Exception as e:
        logger.warning(f"Файл {os.path.basename(file_path)} повреждён: {e}")
        return False

def main():
    xml_dir = r'U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\BASE'
    
    # --- НАЧАЛО: проверка XML-файлов до перемещения ---
    logger.info("Начало проверки XML-файлов в директории: %s", xml_dir)
    
    # Получаем список всех XML-файлов в исходной папке
    xml_files = glob.glob(os.path.join(xml_dir, '*.xml'))
    if not xml_files:
        logger.info("XML-файлы не найдены. Программа продолжит работу.")
    else:
        # Цикл проверки, пока все файлы не станут валидными
        while True:
            bad_files = []
            for f in xml_files:
            ...
            if not bad_files:
                logger.info("Все XML-файлы валидны.")
                break
            
            # Вывод предупреждения на экран
            print("\n" + "="*60)
            print("ВНИМАНИЕ! Обнаружены повреждённые XML-файлы:")
            for name in bad_files:
                print(f"  - {name}")
            print("Пожалуйста, исправьте эти файлы или удалите их из папки.")
            print("После исправления нажмите Enter для продолжения...")
            print("="*60)
            
            # Ожидание ввода от пользователя
            input()
            # После ввода цикл повторится – проверка заново
            # Перечитаем список актуальных файлов на случай удаления/добавления.
            xml_files = glob.glob(os.path.join(xml_dir, '*.xml'))
            logger.info("Повторная проверка после вмешательства пользователя...")
